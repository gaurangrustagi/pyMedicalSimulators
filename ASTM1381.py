...
Config_FrameSize = 240
# MAX_FRAME_CHAR = 240


def encodeByte(frame):
	return frame.encode( "windows-1252" ) 
...
class Device:
	...
	def Send( self, message ):
		frames = message.Output()
		transmition_success = True
		self.Socket.send(encodeByte(ENQ))
		if(len(self.Socket.recv( 1024 )) != 0):
			for frame in frames:
				self.Socket.send(encodeByte(frame) )
				data = self.Socket.recv( 1024 )
				if(len(data) == 0):
					transmition_success = False
					break
		if(transmition_success):
			self.Socket.send(encodeByte(EOT) )
		data = self.Socket.recv( 1024 )
		print(data)
		return data
	def Close( self ):
